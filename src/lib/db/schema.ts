...
import { bookingStatusEnum, damageSeverityEnum, damageStatusEnum, equipmentConditionEnum, equipmentStatusEnum, notificationTypeEnum, roleRequestStatusEnum } from "@/utils/extraForSchema";
...
export const RoleRequestTable = pgTable("role_requests", {
...
export const userRelations = relations(user, ({ many }) => ({
  ...
  roleRequests: many(RoleRequestTable, { relationName: "user_role_requests" }),
  reviewedRoleRequests: many(RoleRequestTable, { relationName: "reviewed_role_requests" })
}));

export const roleRequestRelations = relations(RoleRequestTable, ({ one }) => ({
  user: one(user, {
    fields: [RoleRequestTable.userId],
    references: [user.id],
    relationName: "user_role_requests"
  }),
  reviewer: one(user, {
    fields: [RoleRequestTable.reviewedById],
    references: [user.id],
    relationName: "reviewed_role_requests"
  })
}));
